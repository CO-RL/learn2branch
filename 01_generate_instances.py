...
def generate_capacited_facility_location(random, filename, n_customers, n_facilities, ratio):
    ...
    # adjust capacities according to 长度。
...
if __name__ == '__main__':
    ...
    if args.problem == 'setcover':
        ...
        # train instances
        n = 10000
        lp_dir = f'data/instances/p_median/train_{number_of_customers}_{number_of_facilities}_{p}'
        print(f"{n} instances in {lp_dir}")
        os.makedirs(lp_dir)
        filenames.extend([os.path.join(lp_dir, f'instance_{i+1}.lp') for i in range(n)])   #extend() 函数用于在列表末尾一次性追加另一个序列中的多个值
        ncustomerss.extend([number_of_customers] * n)
        ...
        # validation instances
        n = 2000
        lp_dir = f'data/instances/p_median/valid_{number_of_customers}_{number_of_facilities}_{p}'
        print(f"{n} instances in {lp_dir}")
        os.makedirs(lp_dir)
        filenames.extend([os.path.join(lp_dir, f'instance_{i+1}.lp') for i in range(n)])
        ncustomerss.extend([number_of_customers] * n)
        ...
        number_of_customers = 100
        number_of_facilities = 100
        lp_dir = f'data/instances/p_median/test_{number_of_customers}_{number_of_facilities}_{p}'
        print(f"{n} instances in {lp_dir}")
        os.makedirs(lp_dir)
        filenames.extend([os.path.join(lp_dir, f'instance_{i+1}.lp') for i in range(n)])
        ncustomerss.extend([number_of_customers] * n)
        ...
        # train instances
        n = 10000
        lp_dir = f'data/instances/TSP/train_{number_of_customers}'
        print(f"{n} instances in {lp_dir}")
        os.makedirs(lp_dir)
        filenames.extend([os.path.join(lp_dir, f'instance_{i+1}.lp') for i in range(n)])   #extend() 函数用于在列表末尾一次性追加另一个序列中的多个值
        ncustomerss.extend([number_of_customers] * n)
        ...
        # validation instances
        n = 2000
        lp_dir = f'data/instances/TSP/valid_{number_of_customers}'
        print(f"{n} instances in {lp_dir}")
        os.makedirs(lp_dir)
        filenames.extend([os.path.join(lp_dir, f'instance_{i+1}.lp') for i in range(n)])
        ncustomerss.extend([number_of_customers] * n)
        ...
        n = 2000
        number_of_customers = 20
        lp_dir = f'data/instances/TSP/test_{number_of_customers}'
        print(f"{n} instances in {lp_dir}")
        os.makedirs(lp_dir)
        filenames.extend([os.path.join(lp_dir, f'instance_{i+1}.lp') for i in range(n)])
        ncustomerss.extend([number_of_customers] * n)
        ...
        for filename, ncs, nsm in zip(filenames, ncustomerss, nsalesmans): #zip() 函数用于将可迭代的对象作为参数，
                                                                                        #将对象中对应的元素打包成一个个元组，然后返回由这些元组组成的列表
            print(f"  generating file {filename} ...")
            generate_MTSP(rng, filename, n_customers=ncs, m_salesmans=nsm)
        ...
        for filename, ncs, nsm in zip(filenames, ncustomerss, nsalesmans): #zip() 函数用于将可迭代的对象作为参数，
                                                                                        #将对象中对应的元素打包成一个个元组，然后返回由这些元组组成的列表
            print(f"  generating file {filename} ...")
            generate_MINMAX_MTSP(rng, filename, n_customers=ncs, m_salesmans=nsm)
